...
class Game:
    ...
    def play_round(self):
        ...
        wining_spin_text = Text(f"The winning number is {winning_number} and its color is {winning_color}")
        wining_spin_text.stylize("bold blue")
        player_panel = Panel(wining_spin_text, title="Winning number", style="on bright_white", border_style="black", width=80)
    ...
    def print_player_balances(self):
        for player in self.players:
            text_player_balances = Text(f"{player.name}'s balance: {player.balance} (bet type: {bet_type}) (bet value: {bet_value}) (bet amount: {bet_amount})")
            text_player_balances.stylize("bold blue")  
            text_box = Panel(text_player_balances, title="Player Balances", style="on bright_white", border_style="black", width=80) 
...
if __name__ == "__main__":
    ...
    text_box = Panel(text, title="Roulette Game", style="on bright_white", border_style="black", width=80) 
    console.print(text_box, justify="center")
    ...
    for i in range(num_players):
        ...
        player_balance = int(input(f"Enter the starting balance for {player_name}: "))

        player_info = Text(f"Player {i + 1}: {player_name} (Balance: {player_balance})")
        player_info.stylize("bold blue")
        player_panel = Panel(player_info, title="Player info", style="on bright_white", border_style="black", width=80)
        console.print(player_panel, justify="center")
        ...
        data = [
            [f"{i} ({colors[i]})", f"{i + 1} ({colors[i + 1]})", f"{i + 2} ({colors[i + 2]})"]
        ...
        text = Text("'number x 10' (press 1) \n 'color x 2' (press 2) \n 'even x 4' (press 3) \n 'odd x 4' (press 4)")
        text.stylize("bold blue")  
        text_box = Panel(text, title="Available bet types", style="on bright_white", border_style="black", width=80) 
        console.print(text_box, justify="center")
        ...
        text = Text(f"Player {i + 1}: {player_name} (bet type: {bet_type}) (bet value: {bet_value}) (bet amount: {bet_amount})")
        text.stylize("bold blue")  
        text_box = Panel(text, title="Bet Detail", style="on bright_white", border_style="black", width=80) 
        console.print(text_box, justify="center")
